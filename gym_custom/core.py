...
class GoalEnv(Env):
    ...
    def reset(self):
        ...
        if not isinstance(self.observation_space, gym_custom.spaces.Dict):
            raise error.Error('GoalEnv requires an observation space of type gym_custom.spaces.Dict')
...
# pickle하고자 하는 class의 attr, method name, state등만 저장해서 dump하고 나중에 load할떄도 걔네들만 가지고 class 새로 만든다 생각!
# 따라서 randomness를 가지는 (e.x. random initialize되는 param) attr까지 똑같이 되는건 아님. 새로 만드는거니까!
